 ...
 * --------------------------------------------
 * Last Modified: 2022-09-29 16:36:33
 * Modified By: ZZQ
 */
...
@interface ImageCropPicker ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,TZImagePickerControllerDelegate>
...
- (NSString*)getTmpDirectory {
    NSString *TMP_DIRECTORY = @"flutter-image-picker/";
...
//			BOOL multiple = [[[self options] objectForKey:@"enableMutipleSelect"] boolValue];
//			imagePickerController.minImagesCount = abs([[self.options objectForKey:@"minFiles"] intValue]);
//			imagePickerController.maxImagesCount = multiple ? abs([[self.options objectForKey:@"maxSelectCount"] intValue]) : 1;
//			imagePickerController.showSelectBtn = NO;
//			imagePickerController.autoSelectCurrentWhenDone = YES;
            if ([[self.options objectForKey:@"cropping"] boolValue]) {
				...
				if ([mediaType isEqualToString:@"photo"]) {
					...
					imagePickerController.allowTakeVideo = YES;
...
//- (void)openCropper:(NSDictionary *)options
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    ...
    if ([[[self options] objectForKey:@"enableMutipleSelect"] boolValue]) {
...
//        [self cropImage:[image fixOrientation]];
        ...
        [viewController dismissViewControllerAnimated:YES completion:[self waitAnimationEnd:^{
            self.resolve(@[
				[self createAttachmentResponse:filePath
												   withExif:exif
